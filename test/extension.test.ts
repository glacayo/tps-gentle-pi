...
test("the parent render tick resolves the four Pi theme colors and forwards them", async (t) => {
  ...
  const headerLine =
    (calls.setWidget[0]?.lines as string[] | undefined)?.[0] ?? "";
  const mainLine =
    (calls.setWidget[0]?.lines as string[] | undefined)?.[1] ?? "";
  ...
  assert.ok(
    headerLine.includes(`${THEME_PREFIXES.text}Throughput`) ||
      mainLine.includes(`${THEME_PREFIXES.text}1 active`),
    headerLine,
  );
  assert.ok(
