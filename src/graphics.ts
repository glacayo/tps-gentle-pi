 ...
 * the unfilled remainder is the track `·`. Rate 0 renders all track; the
 * GAUGE_MAX_TPS ceiling renders all full blocks. Out-of-range and non-finite
 * rates clamp to the empty/full boundaries.
 */
export function formatGauge(tps: number, cells = DEFAULT_GAUGE_CELLS): string {
 const cellCount = Math.max(1, Math.floor(cells));
 const rate = Number.isFinite(tps) ? clamp(tps, 0, GAUGE_MAX_TPS) : 0;

 const eighths = Math.round((rate / GAUGE_MAX_TPS) * cellCount * 8);
